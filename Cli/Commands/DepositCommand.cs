...
namespace Cli.Commands
{
    public class DepositCommand : Command
    {
        public override int Execute(CommandContext context)
        {
            ...
            {
                CliWidgets.RenderError("Enter a valid integer.\nNote that amounts greater than Q 2 147 483 647 can't be deposited via the Cli, please visit the nearest galaxy bank.");
                return 1;
            }
            if (amount <= 0)
            {
            ...
            try
            {
                ...
                var result = httpClient.PostAsync($"{Constants.ApiBaseUrl}/deposits", content).Result;

                if (result.IsSuccessStatusCode)
                {
                    CliWidgets.RenderPanel($"[green]Deposited Q {amount:n0} to account {accountNumber} with reference {reference}[/]");
                    return 0;
                }
                else
                {
                    var errorMessage = result.Content.ReadAsStringAsync().Result;
                    CliWidgets.RenderError($"[red]Failed to deposit: {(int)result.StatusCode} - {result.ReasonPhrase} - {errorMessage}[/]");
                    return 1;
                }
            }
            catch (Exception ex)
