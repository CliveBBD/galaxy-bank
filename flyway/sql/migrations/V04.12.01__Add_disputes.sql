-- Dispute Seeding Script
BEGIN;

-- Set a random seed for consistent results
SELECT setseed(0.5);

-- Temporary table to hold transactions we'll dispute
CREATE TEMP TABLE candidate_transactions AS
WITH ranked_transactions AS (
  ...
  WHERE t.transaction_type_id IN (
    SELECT transaction_type_id FROM transaction_types WHERE name IN ('deposit', 'withdrawal', 'transfer')
  )
...
SELECT 
  CASE 
    WHEN random() < 0.3 THEN 'Unauthorized transaction'
    WHEN random() < 0.6 THEN 'Incorrect amount charged'
    WHEN random() < 0.8 THEN 'Duplicate charge'
    ELSE 'Service not received'
  END as reason,
...
FROM disputes d
JOIN candidate_transactions ct ON d.disputed_transaction_reference_id = ct.transaction_reference_id;
...
-- For accepted disputes, we need to:
-- 1. Create reversal transactions
-- 2. Update account balances

-- First create the reversal transaction types if they don't exist
INSERT INTO transaction_types (name)
SELECT 'reversal' WHERE NOT EXISTS (SELECT 1 FROM transaction_types WHERE name = 'reversal');
...
SELECT 
  -- Use the original transaction's reference ID to link them
  d.disputed_transaction_reference_id,
  'Dispute reversal for #' || ct.transaction_id as reference,
  ct.account_id,
